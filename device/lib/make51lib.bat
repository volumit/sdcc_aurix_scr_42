...
set MODELS=medium large

for %%M in (%MODELS%) do (
  pushd %%M
  del *.a
  set SDCCLIB_CC=sdcc --use-stdout --model-%%M -c
  %AR% -S -cq libfloat.a %REL_FLOAT%
  %RANLIB% libfloat.a
  %AR% -S -cq libint.a %REL_INT%
  %RANLIB% libint.a
  %AR% -S -cq liblong.a %REL_LONG%
  %RANLIB% liblong.a
  %AR% -S -cq libsdcc.a %REL_SDCC%
  %RANLIB% libsdcc.a
  cd ..\mcs51
  for %%I in (*.asm) do sdas8051 -plosgff %%I
  for %%I in (*.rel) do %AR% -S -cq ..\%%M\libmcs51.a %%I
  %RANLIB% ..\%%M\libmcs51.a
  popd
)
endlocal
