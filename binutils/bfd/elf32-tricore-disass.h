 ...
 * Minor adaptations for reuse in bfd tricore - 2020/22/02
 */

#ifndef BFD_ELF32_TRICORE_DISASS_H_
#define BFD_ELF32_TRICORE_DISASS_H_
...
{
  BFD_ASSERT(start >= 0 && length > 0 && length <= 32 - start);
    return (value >> start) & (~0U >> (32 - length));
}

static inline uint32_t insert32(uint32_t value, int start, int length, uint32_t reg)
{
unsigned int insn;
BFD_ASSERT(start >= 0 && length > 0 && length <= 32 - start);
insn=~((~0U >> (32 - length))<<start);
insn&=value;
insn|=(reg<<start);
return(insn);
BFD_ASSERT(start >= 0 && length > 0 && length <= 32 - start);
...
static inline uint32_t sinsert32_bol(uint32_t value, int offs)
{
value&=0x0000FFFF;
value |= ((offs & 0x3f) << 16);
value |= ((offs & 0x3c0) << 22);
value |= ((offs & 0xfc00) << 12);
return value;
}
...
#define MASK_BITS_SHIFT(op, start, end) (extract32(op, (start), \
                                        (end) - (start) + 1))
#define MASKINS_BITS_SHIFT(op, start, end, reg) (insert32(op, (start), \
                                        (end) - (start) + 1,reg))

#define MASK_BITS_SHIFT_SEXT(op, start, end) (sextract32(op, (start),\
                                             (end) - (start) + 1))
#define MASKINS_BITS_SHIFT_SEXT(op, start, end, reg) (sinsert32(op, (start), \
                                        (end) - (start) + 1,reg))
...
#define MASK_OP_B_DISP22_SEXT(op)   (MASK_BITS_SHIFT(op, 16, 31) + \
                                    (MASK_BITS_SHIFT_SEXT(op, 8, 13) << 16))
/* BIT Format */
...
#define MASKINS_OP_BIT_D(op,reg)      MASKINS_BITS_SHIFT(op, 28, 31,reg)
#define MASKINS_OP_BIT_S2(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_BIT_S1(op,reg)     MASKINS_BITS_SHIFT(op, 8, 11,reg)
...
/* 16-Bit Formats */
#define MASKINS_OP_SB_DISP8(op,reg)   MASKINS_BITS_SHIFT(op, 8, 15,reg)
#define MASKINS_OP_SB_DISP8_SEXT(op,reg) MASKINS_BITS_SHIFT_SEXT(op, 8, 15,reg)

#define MASKINS_OP_SBC_CONST4(op,reg) MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SBC_CONST4_SEXT(op,reg) MASKINS_BITS_SHIFT_SEXT(op, 12, 15,reg)
#define MASKINS_OP_SBC_DISP4(op,reg)  MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SBR_S2(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SBR_DISP4(op,reg)  MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SBRN_N(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SBRN_DISP4(op,reg) MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SC_CONST8(op,reg)  MASKINS_BITS_SHIFT(op, 8, 15,reg)

#define MASKINS_OP_SLR_S2(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SLR_D(op,reg)      MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SLRO_OFF4(op,reg)  MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SLRO_D(op,reg)     MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SR_OP2(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SR_S1D(op,reg)     MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SRC_CONST4(op,reg) MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SRC_CONST4_SEXT(op,reg) MASKINS_BITS_SHIFT_SEXT(op, 12, 15,reg)
#define MASKINS_OP_SRC_S1D(op,reg)    MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SRO_S2(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SRO_OFF4(op,reg)   MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SRR_S2(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SRR_S1D(op,reg)    MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SRRS_S2(op,reg)    MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SRRS_S1D(op,reg)   MASKINS_BITS_SHIFT(op, 8, 11,reg)
#define MASKINS_OP_SRRS_N(op,reg)     MASKINS_BITS_SHIFT(op, 6, 7,reg)

#define MASKINS_OP_SSR_S2(op,reg)     MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SSR_S1(op,reg)     MASKINS_BITS_SHIFT(op, 8, 11,reg)

#define MASKINS_OP_SSRO_OFF4(op,reg)  MASKINS_BITS_SHIFT(op, 12, 15,reg)
#define MASKINS_OP_SSRO_S1(op,reg)    MASKINS_BITS_SHIFT(op, 8, 11,reg)

/* 32-Bit Formats */

/* ABS Format */
#define MASKINS_OP_ABS_S1D(op,reg)    MASKINS_BITS_SHIFT(op, 8, 11,reg)
...
/* BO Format */
#define MASKINS_OP_BO_S2(op, reg)      MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_BO_S1D(op, reg)     MASKINS_BITS_SHIFT(op, 8, 11, reg)

/* BOL Format */
#define MASKINS_OP_BOL_S2(op, reg)     MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_BOL_S1D(op, reg)    MASKINS_BITS_SHIFT(op, 8, 11, reg)
#define MASKINS_OP_BOL_OFF16_SEXT(op, offs) sinsert32_bol(op,offs)
/* BRC Format */
#define MASKINS_OP_BRC_S1(op, reg)     MASKINS_BITS_SHIFT(op, 8, 11, reg)

/* BRN Format */
#define MASKINS_OP_BRN_S1(op, reg)     MASKINS_BITS_SHIFT(op, 8, 11, reg)
/* BRR Format */
#define MASKINS_OP_BRR_S2(op, reg)     MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_BRR_S1(op, reg)     MASKINS_BITS_SHIFT(op, 8, 11, reg)

/* META MASK for similar instr Formats */
#define MASKINS_OP_META_D(op, reg)     MASKINS_BITS_SHIFT(op, 28, 31, reg)
#define MASKINS_OP_META_S1(op, reg)    MASKINS_BITS_SHIFT(op, 8, 11, reg)

/* RC Format */
#define MASKINS_OP_RC_D(op, reg)       MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RC_S1(op, reg)      MASKINS_OP_META_S1(op, reg)

/* RCPW Format */

#define MASKINS_OP_RCPW_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RCPW_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RCR Format */

#define MASKINS_OP_RCR_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RCR_S3(op, reg)     MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RCR_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RCRR Format */

#define MASKINS_OP_RCRR_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RCRR_S3(op, reg)     MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RCRR_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RCRW Format */

#define MASKINS_OP_RCRW_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RCRW_S3(op, reg)     MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RCRW_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RLC Format */

#define MASKINS_OP_RLC_D(op, reg)       MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RLC_S1(op, reg)      MASKINS_OP_META_S1(op, reg)

/* RR  Format */
#define MASKINS_OP_RR_D(op, reg)        MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RR_S2(op, reg)       MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RR_S1(op, reg)       MASKINS_OP_META_S1(op, reg)

/* RR1  Format */
#define MASKINS_OP_RR1_D(op, reg)       MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RR1_S2(op, reg)      MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RR1_S1(op, reg)      MASKINS_OP_META_S1(op, reg)

/* RR2  Format */
#define MASKINS_OP_RR2_D(op, reg)       MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RR2_S2(op, reg)      MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RR2_S1(op, reg)      MASKINS_OP_META_S1(op, reg)

/* RRPW  Format */
#define MASKINS_OP_RRPW_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RRPW_S2(op, reg)     MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RRPW_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RRR  Format */
#define MASKINS_OP_RRR_D(op, reg)       MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RRR_S3(op, reg)      MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RRR_S2(op, reg)      MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RRR_S1(op, reg)      MASKINS_OP_META_S1(op, reg)

/* RRR1  Format */
#define MASKINS_OP_RRR1_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RRR1_S3(op, reg)     MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RRR1_S2(op, reg)     MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RRR1_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RRR2  Format */
#define MASKINS_OP_RRR2_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RRR2_S3(op, reg)     MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RRR2_S2(op, reg)     MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RRR2_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RRRR  Format */
#define MASKINS_OP_RRRR_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RRRR_S3(op, reg)     MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RRRR_S2(op, reg)     MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RRRR_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* RRRW  Format */
#define MASKINS_OP_RRRW_D(op, reg)      MASKINS_OP_META_D(op, reg)
#define MASKINS_OP_RRRW_S3(op, reg)     MASKINS_BITS_SHIFT(op, 24, 27, reg)
#define MASKINS_OP_RRRW_S2(op, reg)     MASKINS_BITS_SHIFT(op, 12, 15, reg)
#define MASKINS_OP_RRRW_S1(op, reg)     MASKINS_OP_META_S1(op, reg)

/* SYS Format */
#define MASKINS_OP_SYS_OP2(op, reg)     MASKINS_BITS_SHIFT(op, 22, 27, reg)
#define MASKINS_OP_SYS_S1D(op, reg)     MASKINS_OP_META_S1(op, reg)
...
enum {
    ...
    OPC1_16_SRC_ADD_A15B                             = 0x92,
    OPC1_16_SRC_ADD_15AB                             = 0x9a,
    OPC1_16_SRR_ADD                                  = 0x42,
    OPC1_16_SRR_ADD_A15B                             = 0x12,
    OPC1_16_SRR_ADD_15AB                             = 0x1a,
...
/* BOL Format */
    ...
    OPC1_32_BOL_LDPSTP_D_LONGOFF                      = 0xc1, /* experimental */
/* BRC Format */
...
#endif /* BFD_ELF32_TRICORE_DISASS_H_ */
