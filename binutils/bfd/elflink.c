		       ...
		       bfd_boolean *size_change_ok)
{
  ...
  *sym_hash = h;
...
#ifdef HTC_SUPPORT
  /* but not if the old undefined symbol has a size or a type 
     other than STT_NOTYPE we will check the size for undefined symbols 
     declared with .extern name,type,size */
#endif
  ...
  if (*type_change_ok
       || (h->root.type == bfd_link_hash_undefined
#ifdef HTC_SUPPORT
          && (h->size == 0 || h->size == -1u || h->type != STT_OBJECT)
#endif
       ))
    *size_change_ok = TRUE;
  ...
  for (isym = isymbuf, isymend = isymbuf + extsymcount;
      ...
      if (is_elf_hash_table (htab))
	{
	  ...
	  if ((isym->st_size != 0 && isym->st_shndx != SHN_UNDEF)
	      && (definition || h->size == 0 ))
	    {
              /* size -1u: the size of the object is unknown i.e. int arr[]
                 or something else, we may change the size */
	      if (h->size != 0 
#ifdef HTC_SUPPORT
                  && h->size != -1u
#endif
	  ...
	  if ((isym->st_size != 0) && (isym->st_shndx == SHN_UNDEF)
            ...
            {
              /* if we have an undefined symbol with size information
                 (declared with .extern) save it */
              if (h->size != 0
                  && h->size != -1u
                  && isym->st_size != -1u
                  && h->size != isym->st_size)
              ...
              /* BIN-63 only propagate isym size != -1 otherwise take the
                 existing symbol size */ 
              if ((h->size == 0 || h->size == -1u) && (isym->st_size != -1u))
                h->size = isym->st_size;
            }
#endif
...
#ifdef HTC_SUPPORT
                  || (ELF_ST_TYPE(isym->st_info) != h->type)
#endif
                  ))
	    {
				 ...
				 bfd *bbfd, const asection *bsec)
{
  ...
  if (elf_section_type (asec) == elf_section_type (bsec))
    {
      if (bed->match_sections_by_type)
        return (*bed->match_sections_by_type)(abfd,asec,bbfd,bsec);
      else
        return TRUE;
    }
  return FALSE;
	    ...
	    if (sym.st_shndx == SHN_BAD)
	      {
                ...
                if (!strncmp(h->root.root.string,"_SMALL_DATA_",12)
                    || !strncmp(h->root.root.string,"_SMALL_DATA2_",13)
                    || !strncmp(h->root.root.string,"_SMALL_DATA3_",13)
                    || !strncmp(h->root.root.string,"_SMALL_DATA4_",13)
                    )
                    return TRUE;
#endif
		(*_bfd_error_handler)
  ...
  for (isym = isymbuf, pindex = finfo->indices, ppsection = finfo->sections;
	...
	continue;
	  ...
	  && isym->st_shndx < SHN_LORESERVE
        ...
        {
          (*_bfd_error_handler)
           (_("%B: no output section assigned to %B(%A)"),
	   output_bfd, isec->owner,isec );
          return FALSE;
        }
#endif
