...
tricore_convert_eflags(unsigned long eflags)
{
  ...
  if (e_flags)
	...
	default:
	  break;
        ...
        case EM_TRICORE:
          switch (tricore_convert_eflags(e_flags) & EF_EABI_TRICORE_CORE_MASK)
            {
...
usage (FILE * stream)
{
  ...
  -w[lLiaprmfFsoR] or\n\
  --debug-dump[=rawline,=decodedline,=info,=abbrev,=pubnames,=aranges,=macro,=frames,=str,=loc,=Ranges,=control_flow]\n\
  ...
  flags [] =
    {
      ...
      /* TRICORE */
      /* 21 */ { STRING_COMMA_LEN ("PCP") },
      /* 22 */ { STRING_COMMA_LEN ("SMALL") }
    };
  ...
  while (sh_flags)
      ...
      if (do_section_details)
	{
...
#ifdef HTC_SUPPORT
                case EM_TRICORE:
                  if (flag == SHF_TRICORE_PCP)
                    index = 21;
                  else if (flag == SHF_TRICORE_SMALL)
                    index = 22;
                  break;
#ifdef HTC_PPCVLE
                case EM_PPC:
	  ...
	  if (index != -1)
	      ...
	      if (p != buff + field_size + 4)
		{
		  if (size < (10 + 2))
                    {
                      error (_("Internal buffer overflow\n"));
                      exit (1);
                    }
		  size -= 2;
		  *p++ = ',';
      ...
      else
	{
	  switch (flag)
	    ...
	    case SHF_SMALL_DATA:	*p = 's'; break;
#endif
#ifdef HTC_TRICORE
            case SHF_TRICORE_PCP:       *p = 'P'; break;
...
#ifdef HTC_SUPPORT
	      else if (flag & SHF_CORE_NUMBER_MASK)
                {
                  *p = 0x30 + SHF_CORE_NUMBER_GET(sh_flags|flag) - 1;
                  sh_flags &= ~SHF_CORE_NUMBER_MASK;
                }
#endif
	      else if (flag & SHF_MASKPROC)
		{
...
#ifdef HTC_SUPPORT
      /* printf the attached core number */
      if (proc_flags & SHF_CORE_NUMBER_MASK)
        {
          unsigned int core = SHF_CORE_NUMBER_GET(proc_flags);
          size -= 6;
          if (p != buff + 5)
            {
	      if (size < (2 + 1))
	      ...
	      *p++ = ' ';
          ...
          if (core > 0 && core < NUM_OF_CORES)
            sprintf(p,"%s", corenames[core]);
          else
            sprintf(p,"CORE_%d",core);
          proc_flags &= ~SHF_CORE_NUMBER_MASK;
          p += 6;
        }
#endif
      if (proc_flags)
...
#define CHECK_ENTSIZE(section, i, type) \
  ...
  for (i = 0, section = section_headers;
                   ...
                   || const_strneq (name, ".zdebug_")))
	{
	  ...
	  if (do_debugging
	      ...
	      || (do_debug_control_flow && streq (name, "control_flow"))
	      )
  ...
  W (write), A (alloc), X (execute), V (VLE section), M (merge), S (strings)\n\
  I (info), L (link order), G (group), O (extra OS processing required) P (PCP section)\n\
  o (OS specific), p (processor specific) x (unknown)\n\
  s (small data), z (absolute data), # (core number)\n"));
#else
  if (!do_section_details)
    printf (_("Key to Flags:\n\
...
get_symbol_visibility (unsigned int visibility)
{
  switch (visibility)
    ...
    default:
                        error (_("Illegal visibility attribute %d\n"),visibility);
                        exit (1);
    }
}
...
static const char *
get_htc_symbol_other(unsigned int other)
{
  unsigned int core;
  static char buffer[16]; 

  core = ELF_STO_READ_CORE_NUMBER(other);
  snprintf(buffer,16,"%s",corenames[core]);
  return buffer;
}
...
get_symbol_other (unsigned int other)
{
  ...
  switch (elf_header.e_machine)
    ...
    case EM_TRICORE:
    case EM_PPC:
      result = get_htc_symbol_other(other);
    default:
      break;
    }
	   ...
	   i++, section++)
	{
	  ...
	  for (si = 0, psym = symtab;
	       ...
	       si++, psym++)
	    {
	      ...
	      printf (" %-6s", get_symbol_binding (ELF_ST_BIND (psym->st_info)));
	      printf (" %-8s", get_symbol_visibility (ELF_ST_VISIBILITY (psym->st_other)));
	      /* Check to see if any other bits in the st_other field are set.
	         Note - displaying this information disrupts the layout of the
...
static int
uncompress_section_contents (unsigned char ** buffer ATTRIBUTE_UNUSED, dwarf_size_type * size ATTRIBUTE_UNUSED)
{
#ifndef HAVE_ZLIB_H
...
process_note (Elf_Internal_Note * pnote)
{
  ...
  if (const_strneq(name,"APUinfo"))
    {
      unsigned int i;
      unsigned short ps;
      for (i = 0; i < pnote->descsz; i += 4)
        {
          ps = byte_get((unsigned char *)&pnote->descdata[i],2);
          switch (ps)
            {
            case 0x104:
              printf ("  APU #0x%x = VLE, revision %d\n",ps,(int)byte_get((unsigned char *)&pnote->descdata[i+2],2));
              break;
            case 0x105:
              printf ("  APU #0x%x = EFPU, revision %d\n",ps,(int)byte_get((unsigned char *)&pnote->descdata[i+2],2));
              break;
            case 0x106:
              printf ("  APU #0x%x = LSP, revision %d\n",ps,(int)byte_get((unsigned char *)&pnote->descdata[i+2],2));
              break;
            case 0x107:
              printf ("  APU #0x%x = Zen42X, revision %d\n",ps,(int)byte_get((unsigned char *)&pnote->descdata[i+2],2));
              break;
            default:
              printf ("  APU #0x%x, revision %d\n",ps,(int)byte_get((unsigned char *)&pnote->descdata[i+2],2));
              break;
            }
        }
    }
    ...
    if (section->sh_type == SHT_NOTE)
      {
        switch (elf_header.e_machine) 
          ...
          case EM_PPC:
          case EM_ARM:
            if (strcmp(SECTION_NAME (section),".version_info") == 0)
              {
                printf(".version_info section at offset 0x%08lx with length 0x%08lx\n",
