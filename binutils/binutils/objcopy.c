...
/* Specifies the size to use when the output format is carray.  */
/* 4 = 32bit, 2 = 16bit, 1 = 8bit  */
static const char * carray_output_size = NULL;
...
static bfd_boolean carray_comments = FALSE;

/* True if --hinc was passed on the command line.  */
static bfd_boolean carray_header_include = FALSE;

/* True if --xtern was passed on the command line.  */
static bfd_boolean carray_header_extern = FALSE;

/* True if --hstdint was passed on the command line.  */
static bfd_boolean carray_header_stdint = FALSE;
/* True if --closegaps was passed on the command line.  */
static bfd_boolean carray_closegaps = FALSE;
...
enum command_line_switch
  {
    ...
    OPTION_CARRAY_HINC,
    OPTION_CARRAY_XTERN,
    OPTION_CARRAY_HSTDINT,
    OPTION_CARRAY_CLOSEGAPS,
#endif /* without feature: HDP-934 */
    OPTION_SUBSYSTEM
  };
...
#endif
#ifndef WITHOUT_FEATURE_HDP_934
  {"otyp", required_argument, 0, OPTION_CARRAY_OTYP},
  {"osize", required_argument, 0, OPTION_CARRAY_OSIZE},
  {"olbl", required_argument, 0, OPTION_CARRAY_OLBL},
  {"odef", required_argument, 0, OPTION_CARRAY_ODEF},
  {"comments", no_argument, 0, OPTION_CARRAY_COMMENTS},
  {"hinc", no_argument, 0, OPTION_CARRAY_HINC},
  {"xtern", no_argument, 0, OPTION_CARRAY_XTERN},
  {"hstdint", no_argument, 0, OPTION_CARRAY_HSTDINT},
  {"closegaps", no_argument, 0, OPTION_CARRAY_CLOSEGAPS},
#endif /* without feature: HDP-934 */
  {0, no_argument, 0, 0}
};
...
Options available when output target is <carray>:\n\
     ...
     --osize <type>                Define output size in bytes <type=4,2,1> with optional qualifiers for\n\
                                   C array (default is 4 equals 32bit values)\n\
     ...
     --hstdint                     Includes stdint.h into <out-file> and <dfile> (default off)\n\
     --closegaps                   closes gaps betweens sections to generate a continous [out-file]\n\
                                   if not turned on, gaps will be reported as error, defuaul off\n\
...
copy_main (int argc, char *argv[])
{
  ...
  while ((c = getopt_long (argc, argv, "b:B:i:I:j:K:N:s:O:d:F:L:G:R:SpgxXHhVvW:w",
...
#ifndef WITHOUT_FEATURE_HDP_934
        case OPTION_CARRAY_OSIZE:
          carray_output_size = optarg;
          break;
	...
	case OPTION_CARRAY_HSTDINT:
	  carray_header_stdint = TRUE;
          break;

	case OPTION_CARRAY_CLOSEGAPS:
          carray_closegaps = TRUE;
          break;
#endif /* without feature: HDP-934 */
