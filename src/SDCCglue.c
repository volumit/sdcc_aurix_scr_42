  ...
  for (sym = setFirstItem (map->syms); sym; sym = setNextItem (map->syms))
    {
      ...
      if (SPEC_ABSA (sym->etype) && !sym->ival)
      ...
      else
        {
          ...
          if (SPEC_ABSA (sym->etype))
            {
              werrorfl (sym->fileDef, sym->lineDef, E_SET_INITIALIZERS_AT, " _at()", sym->name);
              dbuf_tprintf (&map->oBuf, "\t!org\n", SPEC_ADDR (sym->etype));
              if (map->regsp) werrorfl (sym->fileDef, sym->lineDef, W_EXCESS_INITIALIZERS, "init of sfr not allowed", sym->name);
              if (map->bitsp) werrorfl (sym->fileDef, sym->lineDef, W_EXCESS_INITIALIZERS, "init of bit not allowed", sym->name);
            }
          /* print extra debug info if required */
          if (options.debug)
            {
      ...
      if (IS_AST_SYM_VALUE (expr->left))
        {
          ...
          else if (sclass == S_SFRDATA)
            DCL_TYPE (val->type) = SFRPOINTER;
          ...
          for (i = (le ? 0 : size - 1); le ? (i < size) : (i > -1); i += (le ? 1 : -1))
            {
              if (i == 0)
			    ...
			    if (val->name && strlen (val->name) > 0)
			      {
			        if (i>1) werror (E_INTERNAL_ERROR, __FILE__, __LINE__, "printIvalType got unsupported offset!=1");
			        if (i==1) dbuf_printf (oBuf, "hi_(%s)", val->name);
          ...
          if (sym->ival)
            {
              ...
              if (options.const_seg)
                //named spaces R code/ro and V ram/data, hide the generation of a section statement
                if ((map->dbName!='R') && (map->dbName!='V'))  dbuf_tprintf(oBuf, "\t!area ;xinit sym->ival\n", options.const_seg);
                }
              dbuf_printf (oBuf, "%s:\n", sym->rname);
              ++noAlloc;
              resolveIvalSym (sym->ival, sym->type);
              ...
              if (IS_ARRAY (sym->type) &&
                 ...
                 IS_INT (sym->type->next) && IS_LONG (sym->type->next) && SPEC_CVAL (sym->etype).v_char32))
                {
                  if (map!=xinit)
                    {
                    if (options.const_seg)
                    dbuf_tprintf(oBuf, "\t!area ;xinit !sym->ival1\n", options.const_seg);
                    }
                  dbuf_printf (oBuf, "%s:\n", sym->rname);
...
//                    dbuf_tprintf(oBuf, "\t!areacode\n", options.code_seg);
              ...
              else
                {
                  if (map!=xinit)
                    {
                  if (options.const_seg)
                    dbuf_tprintf(oBuf, "\t!area ;xinit !sym->ival2\n", options.const_seg);
                    }
                  dbuf_printf (oBuf, "%s:\n", sym->rname);
...
emitMaps (void)
{
  ...
  for (nm = namedspacemaps; nm; nm = nm->next)
    if (nm->is_const)
      {
//        dbuf_tprintf (&nm->map->oBuf, "\t!areacode;emitmaps\n", nm->map->sname);
...
//    dbuf_tprintf (&code->oBuf, "\t!area\n", options.const_seg);
    ...
    emitStaticSeg (statsg, &cconst->oBuf);

  if (port->genXINIT)
    {
      //dbuf_tprintf (&code->oBuf, "\t!area\n", xinit->sname);
      emitStaticSeg (xinit, &xinit->oBuf);
    }
  if (initializer)
    {
      dbuf_tprintf (&code->oBuf, "\t!area\n", initializer->sname);
...
//  dbuf_tprintf (vBuf, "\t!areacode\n", HOME_NAME);
  dbuf_printf (vBuf, "\t.globl\t_resetvec\n");
  dbuf_tprintf (vBuf, "\t!areacode\n", ".vect_tab,\"ax\",@progbits");
  dbuf_printf (vBuf, "_resetvec:\n");
  ...
  for (sym = setFirstItem (publics); sym; sym = setNextItem (publics))
    {
      ...
      /* do not global _at qualifiers */
      if (!SPEC_ABSA(sym->etype))  tfprintf (afile, "\t!global\n", sym->rname);
    }
}
  ...
  if (mcs51_like)
    {
      ...
      /* copy the sbit segment */
      if (dbuf_get_length (&sfrbit->oBuf)!=0)
        {
      fprintf (asmFile, "%s", iComments2);
      fprintf (asmFile, "; special function bits\n");
      fprintf (asmFile, "%s", iComments2);
...
//                    fprintf (asmFile, "; overlayable bit register bank\n");
//                    fprintf (asmFile, "%s", iComments2);
//                    fprintf (asmFile, ";\t.section .bitdata.i51,\"aw\" ;area BIT_BANK\t(REL,OVR,DATA)\n");
//                    fprintf (asmFile, ";bits:\n;\t.ds.b 8\n"); //8bits are 8bytes
//                    fprintf (asmFile, ";\tb0 = bits[0]\n");
//                    fprintf (asmFile, ";\tb1 = bits[1]\n");
//                    fprintf (asmFile, ";\tb2 = bits[2]\n");
...
//                    fprintf (asmFile, ";\tb7 = bits[7]\n");
  ...
  if (dbuf_get_length (&data->oBuf)!=0)
    {
  fprintf (asmFile, "%s", iComments2);
  ...
  if (initialized)
    {
      if (dbuf_get_length (&initialized->oBuf)!=0)
        {
      fprintf (asmFile, "%s", iComments2);
  ...
  for (nm = namedspacemaps; nm; nm = nm->next)
    {
      if (dbuf_get_length (&nm->map->oBuf)!=0)
        {
      fprintf (asmFile, "%s", iComments2);
      fprintf (asmFile, "; %s %s data\n", nm->name, nm->is_const ? "rom" : "ram");
      fprintf (asmFile, "%s", iComments2);
//      if (nm->is_const)
//                 tfprintf (asmFile, "\t!areacode ;glue const\n", nm->map->sname);
//      else
//                 tfprintf (asmFile, "\t!areacode ;glue ram\n", nm->map->sname);
      if (nm->is_const)
                 fprintf (asmFile, "\t.section .constnamed.%s,\"ax\"\n", nm->map->sname);
      else
                 fprintf (asmFile, "\t.section .datanamed.%s,\"aw\"\n", nm->map->sname);
...
//    }
  ...
  if (idata)
    {
      if (dbuf_get_length (&idata->oBuf)!=0)
        {
      fprintf (asmFile, "%s", iComments2);
...
//    }
  ...
  if (pdata)
    {
      if (dbuf_get_length (&pdata->oBuf)!=0)
        {
      fprintf (asmFile, "%s", iComments2);
...
//      fprintf (asmFile, "\t.area XSTK (PAG,XDATA)\n" "__start__xstack:\n\t.ds\t1\n\n");
  ...
  if (xdata && (mcs51_like || TARGET_MOS6502_LIKE ))
    {
      if (dbuf_get_length (&xdata->oBuf)!=0)
        {
      fprintf (asmFile, "%s", iComments2);
...
//    }
  ...
  if (xidata)
    {
      if (dbuf_get_length (&xidata->oBuf)!=0)
        {
      fprintf (asmFile, "%s", iComments2);
...
//    }
  ...
  if (dbuf_get_length (&statsg->oBuf)!=0)
    {
  fprintf (asmFile, "%s", iComments2);
  fprintf (asmFile, "; global & static initialisations\n");
  fprintf (asmFile, "%s", iComments2);
  tfprintf (asmFile, "\t!area\n", port->mem.static_name);
  tfprintf (asmFile, ";start of code init sequences\n");
  dbuf_write_and_destroy (&statsg->oBuf, asmFile);
  tfprintf (asmFile, ";end of code init sequences\n");
    }
...
//    }
  /* copy over code */
  if (dbuf_get_length (&code->oBuf)!=0)
    {
  fprintf (asmFile, "%s", iComments2);
...
//  tfprintf (asmFile, ";\t!areacode\n", options.code_seg);
  ...
  if (dbuf_get_length (&cconst->oBuf)!=0)
    {
  fprintf (asmFile, "%s", iComments2);
  fprintf (asmFile, "; const \n");
  fprintf (asmFile, "%s", iComments2);
  //is part of emitstatic seg
  //  tfprintf (asmFile, "\t!area ;*\n", options.const_seg);
  dbuf_write_and_destroy (&cconst->oBuf, asmFile);
    }

  if (dbuf_get_length (&xinit->oBuf)!=0)
    {
  fprintf (asmFile, "%s", iComments2);
  fprintf (asmFile, "; xinit \n");
  fprintf (asmFile, "%s", iComments2);
  tfprintf (asmFile, "\t!area\n", xinit->sname);
  dbuf_write_and_destroy (&xinit->oBuf, asmFile);
    }
