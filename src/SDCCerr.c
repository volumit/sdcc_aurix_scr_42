...
} ErrTab [] =
{
  ...
  { E_SET_INITIALIZERS_AT, ERROR_LEVEL_SYNTAX_ERROR,
        "assigns init elements after %s with in %s", 0 },   
  { E_SET_DIRECT, ERROR_LEVEL_SYNTAX_ERROR,
              "using direct address mode not possible; in file '%s' line number '%d' : %s", 0 },
  { E_MISC, ERROR_LEVEL_SYNTAX_ERROR,
                          "%s", 0 },
};
...
/* -------------------------------------------------------------------------------
 * enabled - Enable output of specified warning
 * -------------------------------------------------------------------------------
 */
void
setWarningEnabled (int errNum)
{
  if ((errNum >= 0) && (errNum < NELEM (ErrTab)) && (ErrTab[errNum].errType <= ERROR_LEVEL_WARNING))
    ErrTab[errNum].disabled = 0;
}
