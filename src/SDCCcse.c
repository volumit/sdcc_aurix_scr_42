...
DEFSETFUNC (findCheaperOpBF)
{
  ...
  if (*opp)
    return 1;
  //fprintf(stderr,"findCheaperOp Entry\n");
  /* not found it yet check if this is the one */
  /* and this is not the defining one          */
  if (cop->key && cop->key == cdp->key)
    {
      ...
      /* constant propagation & dead code elim */
      /* for assignments only                 */
      //fprintf(stderr,"findCheaperOp A1\n");
      if (cdp->diCode->op == '=')
        {
          /* if the result is volatile then return result */
          if (IS_OP_VOLATILE (IC_RESULT (cdp->diCode)))
            {
              //fprintf(stderr,"findCheaperOp A2\n");
              *opp = IC_RESULT (cdp->diCode);
            }
          else
            {
              ...
              if (!POINTER_SET (cdp->diCode) &&
                  ...
                  IS_TRUE_SYMOP (IC_RIGHT (cdp->diCode)))
                {
                  //fprintf(stderr,"findCheaperOp A3\n");
                  *opp = IC_RESULT (cdp->diCode);
                }
              else
                {
                  ...
                  */
                  if (!POINTER_SET (cdp->diCode) &&
                      ...
                      ((!SPIL_LOC (IC_RIGHT (cdp->diCode)) && SPIL_LOC (IC_RESULT (cdp->diCode))) ||
                       (SPIL_LOC (IC_RESULT (cdp->diCode)) && SPIL_LOC (IC_RESULT (cdp->diCode)) == SPIL_LOC (IC_RIGHT (cdp->diCode)))))
                    {
//                      printOperand (IC_RESULT (cdp->diCode), stderr);
                      //fprintf(stderr,"\nfindCheaperOp A4\n");
                      *opp = IC_RESULT (cdp->diCode);
                    }
                  else
                    {
//                      printOperand (IC_RIGHT (cdp->diCode), stderr);
                      //fprintf(stderr,"\nfindCheaperOp A5\n");

                      *opp = IC_RIGHT (cdp->diCode);
//                      if (!isOperandLiteral(*opp))
//                        {
////                        fprintf(stderr,"Not a literal!\n");
//                      *opp=NULL;
//                      return 0;
//                        }
                      if (!isOperandLiteral(*opp))
...
//                      return 0;
        ...
        {
          //fprintf(stderr,"findCheaperOp A6\n");
          *opp = IC_RESULT (cdp->diCode);
        }
    }
  ...
  if (IS_TRUE_SYMOP (cop) &&
      ...
      IS_ITEMP (IC_RESULT (cdp->diCode)))
    {
      //fprintf(stderr,"findCheaperOp A7\n");
      *opp = IC_RESULT (cdp->diCode);
    }
  ...
  if ((*opp) &&
         ...
         (SPEC_LONG(operandType (cop))==SPEC_LONG(operandType (*opp)))))))
    {
      if ((isGlobalInNearSpace (cop) && !isOperandLiteral (*opp)) ||
          isOperandVolatile (*opp, FALSE))
        {
          //fprintf(stderr,"findCheaperOp 1 NO\n");
          *opp = NULL;
          return 0;
        }
      ...
      if (cop->key == (*opp)->key)
        {
          *opp = NULL;
          //fprintf(stderr,"findCheaperOp 2 NO\n");
          return 0;
        }
      ...
      if ((*opp)->isaddr != cop->isaddr && IS_ITEMP (cop))
        {
          *opp = operandFromOperand (*opp);
          (*opp)->isaddr = cop->isaddr;
          //fprintf(stderr,"findCheaperOp A8\n");
        }
      ...
      if (IS_SPEC(operandType (cop)) && IS_SPEC(operandType (*opp))
          ...
          && SPEC_USIGN(operandType(*opp)) != SPEC_USIGN(operandType(cop)))
        {
          //fprintf(stderr,"findCheaperOp A9\n");
          SPEC_USIGN(operandType(*opp)) = SPEC_USIGN(operandType(cop));
        }
          ...
          SPEC_NOUN(operandType(cop)) != SPEC_NOUN(operandType(*opp)))
        {
          ...
          if (isOperandLiteral(*opp) &&
              ...
              SPEC_NOUN(operandType(cop)) == V_INT)
            {
              *opp = operandFromOperand (*opp);
              SPEC_NOUN(operandType(*opp)) = V_INT;
              //fprintf(stderr,"findCheaperOp A10\n");
            }
          else
            {
              // No clue...
              *opp = NULL;
              //fprintf(stderr,"findCheaperOp 3 NO\n");
              return 0;
            }
        }
      //fprintf(stderr,"findCheaperOp 4 YES\n");
      return 1;
    }
  *opp=NULL;
  //fprintf(stderr,"findCheaperOp 5 NO\n");
  return 0;
}
  ...
  if (IS_OP_LITERAL (IC_COND (ic)))
    {
      ...
      /* reducing an if state to a singular condition is no failure/warning */
      /* code stays in, but is always false */
      if (condlitcase==0) werrorfl (ic->filename, ic->lineno, W_CONTROL_FLOW);
      return;
    }
...
//            {
//          fprintf(stderr, "Status here1! %d\n",POINTER_SET (ic));
//          fprintf(stderr, "Status here1! %d\n",IS_SYMOP (IC_RESULT (ic)));
//          fprintf(stderr, "Status here1 BITFIELD taken if 0 %d\n",IS_BITFIELD (OP_SYMBOL (IC_RESULT (ic))->etype));
//          fprintf(stderr, "Status here1 BIT ?? %d\n",IS_BIT (OP_SYMBOL (IC_RESULT (ic))->etype));
//          fprintf(stderr, "Status here1 BITVAR ?? %d\n",IS_BITVAR (OP_SYMBOL (IC_RESULT (ic))->etype));
...
//          if (pdop)
//            {
//              printOperand (pdop, stderr);
//              fprintf(stderr, "\n");
//            }
//          fprintf(stderr, "Status here1 done \n");
//
//            }
          if (POINTER_SET (ic) && IS_SYMOP (IC_RESULT (ic)) &&
              !(IS_BITFIELD (OP_SYMBOL (IC_RESULT (ic))->etype)))
...
//              fprintf(stderr, "was here1!\n");
              ...
              if (pdop && !computeOnly && IS_ITEMP (pdop))
                {
//                  fprintf(stderr, "was here1A Found Cheaper\n");
                  ReplaceOpWithCheaperOp (&IC_RESULT(ic), pdop);
//                  fprintf(stderr, "was here1A Found Cheaper done\n");
                  if (!IS_PTR (operandType (IC_RESULT (ic))))
                    {
                      setOperandType (IC_RESULT (ic),
                                      aggrToPtr (operandType (IC_RESULT (ic)), FALSE));
                    }
//                  fprintf(stderr, "was here1A!\n");
...
//first check if BF and !BF, if cheaper will match
//      if (!computeOnly &&
//            ic->op != ADDRESS_OF &&
//            IS_SYMOP (IC_LEFT (ic)))
//        {
//      fprintf(stderr, "Status here2 BITFIELD if 0 taken ! %d\n",IS_BITFIELD (OP_SYM_ETYPE (IC_LEFT (ic))));
//      fprintf(stderr, "Status here2 BIT ?? %d\n",IS_BIT (OP_SYM_ETYPE (IC_LEFT (ic))));
//      fprintf(stderr, "Status here2 BITVAR ??? %d\n",IS_BITVAR (OP_SYM_ETYPE (IC_LEFT (ic))));
...
//          fprintf(stderr, "\n");
//        }
//      fprintf(stderr, "Status here2 done \n");
//
//        }
      //it is considered as to be woring, no fail so far
      if (!computeOnly &&
            ic->op != ADDRESS_OF &&
            IS_SYMOP (IC_LEFT (ic)) &&
            IS_BITFIELD (OP_SYM_ETYPE (IC_LEFT (ic))))
...
//              IS_SYMOP (IC_LEFT (ic)))
        {
          ...
          //fprintf(stderr, "was here2! %d\n",IS_BITFIELD (OP_SYM_ETYPE (IC_LEFT (ic))));
          //fprintf(stderr, "was here2! %d\n",IS_BIT (OP_SYM_ETYPE (IC_LEFT (ic))));
          //fprintf(stderr, "was here2! %d\n",IS_BITVAR (OP_SYM_ETYPE (IC_LEFT (ic))));
//          fprintf(stderr, "was here2A!\n");
//          printOperand (IC_LEFT (ic), stderr);
//          fprintf(stderr, "\n");
          pdop = NULL;
          applyToSetFTrue (cseSet, findCheaperOp, IC_LEFT (ic), &pdop, checkSign);
          if (pdop)
            {
//              fprintf(stderr, "was here2A Findcheaper Taken!\n");
//              printOperand (pdop, stderr);
//              fprintf(stderr, "\n");
//              fprintf(stderr, "was here2A! Found Cheaper\n");
//              if (DCL_TYPE(operandType(IC_LEFT(ic)))==SFRPOINTER) fprintf(stderr, "was here2A! SFRPOINTER\n");
              const char *iLine;
              iLine = printILine (ic);
//              fprintf(stderr,"%s \n", iLine);
              dbuf_free (iLine);
...
//                            fprintf(stderr, "\n");
              ...
              if (isOperandLiteral(pdop))
                {
//                  fprintf(stderr, "was here2A! Found Cheaper done\n");
...
//                      fprintf(stderr, "was here2A1! Found Cheaper\n");
                      ReplaceOpWithCheaperOp (&IC_LEFT (ic), pdop);
//                      fprintf(stderr, "was here2A1! Found Cheaper done1\n");
                      change = replaced = 1;
                    }
                  ...
                  if (applyToSetFTrue (cseSet, findPointerSet, IC_LEFT (ic), &pdop, IC_RESULT (ic)) &&
                      !bitVectBitValue (ebb->ptrsSet, pdop->key))
                    {
                      ic->op = '=';
                      IC_LEFT (ic) = NULL;
//                      fprintf(stderr, "was here2A2! Found Cheaper\n");
                      ReplaceOpWithCheaperOp (&IC_RIGHT (ic), pdop);
//                      fprintf(stderr, "was here2A2! Found Cheaper done2\n");
                      SET_ISADDR (IC_RESULT (ic), 0);
                      replaced = 1;
                    }
                }
                ...
                {
                  ////fprintf(stderr, "was here2A!\n");
//                  fprintf(stderr, "was here2A3! Found Cheaper\n");
                  ReplaceOpWithCheaperOp (&IC_LEFT (ic), pdop);
//                  fprintf(stderr, "was here2A3! Found Cheaper done3\n");
                  change = replaced = 1;
                }
                }
            }
              ...
              if (POINTER_GET (ic))
                {
                  if (IS_ITEMP (pdop) || IS_OP_LITERAL (pdop))
                    {
                      ...
                      if (bitVectBitValue (ebb->ndompset, IC_LEFT (ic)->key))
                          ebb->ptrsSet = bitVectSetBit (ebb->ptrsSet, pdop->key);
                      //fprintf(stderr, "was here2! Found Cheaper\n");
                      ReplaceOpWithCheaperOp (&IC_LEFT (ic), pdop);
                      //fprintf(stderr, "was here2! Found Cheaper done1\n");
                      change = replaced = 1;
                    }
                  /* check if there is a pointer set
                     for the same pointer visible if yes
                  ...
                  if (applyToSetFTrue (cseSet, findPointerSet, IC_LEFT (ic), &pdop, IC_RESULT (ic)) &&
                      !bitVectBitValue (ebb->ptrsSet, pdop->key))
                    {
                      ...
                      //fprintf(stderr, "was here2! Found Cheaper\n");
                      ReplaceOpWithCheaperOp (&IC_RIGHT (ic), pdop);
                      //fprintf(stderr, "was here2! Found Cheaper done2\n");
                      SET_ISADDR (IC_RESULT (ic), 0);
                      replaced = 1;
                    }
                }
              else
                {
                  ...
                  //fprintf(stderr, "was here2! Found Cheaper\n");
                  ReplaceOpWithCheaperOp (&IC_LEFT (ic), pdop);
                  //fprintf(stderr, "was here2! Found Cheaper done3\n");
                  change = replaced = 1;
                }
            }
        }
      ...
      if (!computeOnly && IS_SYMOP (IC_RIGHT (ic)))
        {
          ...
          if (pdop)
            {
              //fprintf(stderr, "was here4! Found Cheaper\n");
              ReplaceOpWithCheaperOp (&IC_RIGHT (ic), pdop);
              //fprintf(stderr, "was here4! Found Cheaper done\n");
              change = replaced = 1;
            }
        }
//      if (!computeOnly &&
...
//          IS_SYMOP (IC_RESULT (ic)))
//        {
//      fprintf(stderr, "Status here3 BITFIELD taken if 0  %d\n",IS_BITFIELD (OP_SYM_ETYPE (IC_RESULT (ic))));
//      fprintf(stderr, "Status here3 BIT %d\n",IS_BIT (OP_SYM_ETYPE (IC_RESULT (ic))));
//      fprintf(stderr, "Status here3 BITVAR %d\n",IS_BITVAR (OP_SYM_ETYPE (IC_RESULT (ic))));
//      printOperand (IC_RESULT (ic), stderr);
//      fprintf(stderr, "\n");
//      pdop = NULL;
...
//          fprintf(stderr, "\n");
//        }
//      fprintf(stderr, "Status here3 done \n");
//        }
...
//            IS_SYMOP (IC_RESULT (ic)))
        {
          //fprintf(stderr, "was here3!\n");
          pdop = NULL;
          applyToSetFTrue (cseSet, findCheaperOp, IC_RESULT (ic), &pdop, checkSign);
          if (pdop)
            {
              //fprintf(stderr, "was here3A! Found Cheaper\n");
              ReplaceOpWithCheaperOp (&IC_RESULT (ic), pdop);
              //fprintf(stderr, "was here3A! Found Cheaper done\n");
              change = 1;
            }
        }
...
//            IS_SYMOP (IC_RESULT (ic)))
        {
//          fprintf(stderr, "was here3A!\n");
          pdop = NULL;
          applyToSetFTrue (cseSet, findCheaperOpBF, IC_RESULT (ic), &pdop, checkSign);
          if (pdop)
            {
//              fprintf(stderr, "was here3A! Found Cheaper\n");
//              if (DCL_TYPE(operandType(IC_RESULT(ic)))==SFRPOINTER) fprintf(stderr, "was here3A! SFRPOINTER\n");
...
//              fprintf(stderr, "\n");
              if (isOperandLiteral(pdop))
                {
//              fprintf(stderr, "was here3A! Found Cheaper done\n");
//              if (IS_BITVAR (operandType (IC_RESULT (ic))->next)) fprintf(stderr, "was here3A! Result was bitfield\n");
//              (operandType (pdop)->next)->select.s.noun=V_BITFIELD;
              operandType (pdop)->next=operandType (IC_RESULT (ic))->next;
              ReplaceOpWithCheaperOp (&IC_RESULT (ic), pdop);
//              sym_link *next=operandType (IC_RESULT (ic))->next;
//              if (IS_BITVAR (operandType (next))) fprintf(stderr, "was here3A! Result was bitfield\n");
//              operandType (pdop)->next=next;
              change = 1;
                }
            }
        }
      ...
      if (!computeOnly && change)
        {
          //fprintf(stderr, "was here! Changed\n");
          algebraicOpts (ic, ebb);
          while (constFold (ic, cseSet));
          while (boolCast (ic, cseSet));
        }
