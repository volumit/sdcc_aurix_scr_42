                  ...
                  if (implicit) // if not to generic, they have to match
                    {
                      if (!IS_GENPTR (newType) &&
                          !((DCL_TYPE (orgType) == DCL_TYPE (newType)) ||
                            (((DCL_TYPE (orgType) == POINTER) || (DCL_TYPE (orgType) == SFRPOINTER)) && (DCL_TYPE (newType) == IPOINTER))))
                        {
                          errors += werror (E_INCOMPAT_PTYPES);
                        }
                    }
    ...
    case '&':                  /* can be unary */
      ...
      if (!LETYPE (tree))
      ...
      else
        {
          ...
          else if (SPEC_SCLS (LETYPE (tree)) == S_SFRDATA)
            DCL_TYPE (p) = SFRPOINTER;
