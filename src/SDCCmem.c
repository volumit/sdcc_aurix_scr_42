...
initMem ()
{
   ...
   */
  istack = allocMap (0, 0, 0, 0, 0, 0, options.stack_loc, ISTACK_NAME, 'B', IPOINTER);
  ...
  cconst = allocMap (0, 1, 0, 0, 0, 1, options.code_loc, CODE_NAME, 'C', CPOINTER);
  ...
  /* Special function register space :-
     ...
     POINTER-TYPE   -   _NONE_
   */
  sfr = allocMap (1, 0, 0, 1, 0, 0, 0, REG_NAME, 'I', SFRPOINTER);
      ...
      if (!nm)
        {
          ...
          nm->map = nm->is_const ?
            allocMap (0, 1, 0, 0, 0, 1, options.code_loc, symbolspace->name, 'R', CPOINTER) :
            allocMap (0, 0, 0, 1, 0, 0, options.data_loc, symbolspace->name, 'V', POINTER);
...
allocGlobal (symbol * sym)
{
  ...
  if (SPEC_ABSA(sym->etype))  wassertl (1, "__at not supported");
