...
#define OPTION_ENABLE_WARNING       "--enable-warning"
...
static const OPTION optionsTable[] = {
  ...
  {0,   OPTION_ENABLE_WARNING, NULL, "<nnnn> Enable specific warning"},
      ...
      if (argv[i][0] == '-' && argv[i][1] == '-')
        {
          ...
          if (strcmp (argv[i], OPTION_ENABLE_WARNING) == 0)
            {
              int w = getIntArg (OPTION_ENABLE_WARNING, argv, &i, argc);
              setWarningEnabled (w);
              continue;
            }
...
preProcess (char **envp)
{
  if (options.c1mode)
  ...
  else
    {
      ...
      addSet (&preArgvSet, Safe_strdup ("-D__SDCC_{port} -D__SDCC_scr"));
