...
static OPTION _mcs51_options[] =
  {
    ...
    { 0, "--asm-scr", &options.asm_scr, "Assemble for Aurix scr" },
    { 0, "--asm-mcs51", &options.asm_mcs51, "Assemble for MSC51" },
    { 0, NULL }
  };
...
_mcs51_finaliseOptions (void)
{
  ...
  if (!options.noOverlay)
    {
      fprintf(stderr,"error: --nooverlay has to be enabled\n");
      exit(1);
    }
  if ((options.asm_scr==0) && (options.asm_mcs51==0))
    {
      fprintf(stderr,"error: one of the options --asm-scr or --asm-mcs51 has to be set\n");
      exit(1);
    }
  if ((options.asm_scr==1) && (options.asm_mcs51==1))
    {
      fprintf(stderr,"error: both options are set --asm-scr and --asm-mcs51 \n");
      exit(1);
    }
  if (options.asm_scr==1)
    {
      addSet(&preArgvSet, Safe_strdup("-DSDCC_AURIX_SCR"));
    }
  ...
  switch (options.model)
    ...
    case MODEL_SMALL:
      ...
      port->s.ptr_size = 3;
      fprintf(stderr,"error: small model is not supported\n");
      exit(1);
      break;
    case MODEL_MEDIUM:
      ...
      port->s.ptr_size = 3;
      //bug-2385.c generic pointers have to be assigned as well to pdata
      generic->paged = 1;
      fprintf(stderr,"error: medium model is not supported\n");
      exit(1);
      break;
    case MODEL_LARGE:
      ...
      port->s.ptr_size = 3;
      //allow for the moment only large and large stack auto with nooverlay
      break;
    case MODEL_HUGE:
      port->mem.default_local_map = xdata;
      port->mem.default_globl_map = xdata;
      port->s.ptr_size = 3;
      fprintf(stderr,"error: huge model is not supported\n");
      exit(1);
      break;
    default:
      port->mem.default_local_map = data;
      port->mem.default_globl_map = data;
      fprintf(stderr,"error: default model is not supported\n");
      exit(1);
      break;
    }
  //ISTACK is always an IPOINTER and not a POINTER for MCS51 like machines
  //It is also needed to optimize the near pointer handling
  istack->ptrType=IPOINTER;
...
{
  int i;
//will be part of the generic startup
//  dbuf_printf (oBuf, "\t%cjmp\t__sdcc_gsinit_startup\n", options.acall_ajmp?'a':'l');
//  if((options.acall_ajmp)&&(maxInterrupts)) dbuf_printf (oBuf, "\t.ds.b\t1\n");
...
//            dbuf_printf (oBuf, "\t.ds.b\t%d\n", options.acall_ajmp?6:5);
          ...
          dbuf_printf (oBuf, "\t.section .isr%2.2d, \"ax\"\n",i);
          dbuf_printf (oBuf, "\t%cjmp\t%s\n", options.acall_ajmp?'a':'l', interrupts[i]->rname);
        }
      else
        {
...
static void _mcs51_gas_do_assemble (set *asmOptions)
{
    char *buf=NULL;;
    struct dbuf_s dbuf;
    char *s;

    static const char *macroCmd_scr[] = {
        "i51-elf-as","-mscr $1.asm","-o $2",NULL
    };
    ...
    if (fullDstFileName==NULL) dbuf_append (&dbuf, ".o", 2);
    s = shell_escape (dbuf_c_str (&dbuf));


    if (options.asm_scr) buf = buildCmdLine(macroCmd_scr, dstFileName, s, NULL, NULL);
    if (options.asm_mcs51) buf = buildCmdLine(macroCmd_mcs51, dstFileName, s, NULL, NULL);
    if (options.verbose) printf ("gas: %s\n", buf);
    if (sdcc_system(buf)) {
        fprintf(stderr,"Error: Could not Finalize Assembler %s\n",buf);
        fprintf(stderr,"Error: Could not Finalize Assembler %s\n",fullDstFileName);
        Safe_free (buf);
        exit(1);
    }
...
static void
_mcs51_ld_do_link (void)
{
  fprintf(stderr,"Error: not implemented use linker directly and compiler with -c\n");
  exit (1);
}
...
/*    "-plosgffw",  */              /* Options without debug */
    ...
    0,
    ".asm",
    _mcs51_gas_do_assemble      /* do_assemble function */
  },
  {                             /* Linker */
    NULL,
    NULL,
...
#endif
    ";XSTK_PAG_XDATA",           // xstack_name
    ";STACK_DATA",               // istack_name
    ".text.code,\"ax\" ;code_name",   // code_name
    ".ddata.i51,\"aw\" ;data_name",    // data_name
    ".idata.i51,\"aw\" ;idata_name",  // idata_name
    ".pdata.i51,\"aw\" ;pdata_name",  // pdata_name
    ".xdata.i51,\"aw\" ;xdata_name",  // xdata_name
    ".bitdata.i51,\"aw\" ;bit_name",      // bit_name
    ".bdata.i51,\"aw\" ;reg_name",    // reg_name
    ".text.gsinit,\"ax\" ;static_name", // static_name
    ";OSEG_OVR_DATA",            // overlay_name
    ".text.gsfinal,\"ax\";post_static_name", // post_static_name
    ".text.home,\"ax\" ;home_name",   // home_name
    ".xdata.init,\"aw\" ;xidata_name",  // xidata_name - initialized xdata
    ".roxdata.init,\"ax\";xinit_name",               // xinit_name - a code copy of xiseg
    ".text.const,\"ax\" ;const_name", // const_name - const data (code or not)
    ".text.cabs,\"ax\" ;cabs_name",   // cabs_name - const absolute data (code or not)
    ".xdata.abs,\"aw\" ;xabs_name", // xabs_name - absolute xdata/pdata
    ".idata.abs,\"aw\" ;iabs_name", // iabs_name - absolute idata/data
    NULL,                        // name of segment for initialized variables
    NULL,                        // name of segment for copies of initialized variables in code space
    NULL,
    NULL,
    0,                          //code space readonly yes
