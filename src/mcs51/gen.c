...
static char *rb1regs[] =
{
  ...
  "bits_0", "bits_1", "bits_2", "bits_3", "bits_4", "bits_5", "bits_6", "bits_7"
//  "b0", "b1", "b2", "b3", "b4", "b5", "b6", "b7"
};

static char *RegB_B0="bits_0";
...
aopForSym (iCode * ic, symbol * sym, bool result)
{
  ...
  if (sym->onStack || sym->iaccess)
    {
      D (emitcode (";", "aopForSym Stack/Iaccess"));
  ...
  D (emitcode (";", "aopForSym dptr"));
  D (emitcode (";", "aopForSym space %s",space->sname));
      ...
      if (sym->isspilt && sym->usl.spillLoc)
        {
          ...
          D (emitcode (";", "aop issplit/spilloc"));
          aop->size = getSize (sym->type);
          return;
        }

      /* else must be a dummy iTemp */
      D (emitcode (";", "aop AOP_DUMMY"));
      ...
      switch (aop->type)
        ...
        case AOP_IMMD:
          if (aop->aopu.aop_immd.from_cast_remat && opIsGptr (oper) && offset == GPTRSIZE - 1)
            ...
            {
              //werrorfl (ic->filename, ic->lineno, E_SET_NEAR_VAR);
              //if (offset!=1) werrorfl (ic->filename, ic->lineno, E_SET_MISC,"Symbol >65535");
              if (offset!=1) werror (E_INTERNAL_ERROR, __FILE__, __LINE__, "aopget got unsupported aop->type offset!=1, addr>65535");
              //dbuf_printf (&dbuf, "#(%s >> %d)", aop->aopu.aop_immd.aop_immd1, offset * 8);
              dbuf_printf (&dbuf, "#hi_(%s)", aop->aopu.aop_immd.aop_immd1);
            }
          else
            {
              D (emitcode (";", "aopGet2A %s",aop->aopu.aop_immd.aop_immd1));
              dbuf_printf (&dbuf, "#%s", aop->aopu.aop_immd.aop_immd1);
            }
          break;
        ...
        case AOP_DIR:
          if ((SPEC_SCLS (getSpec (operandType (oper))) == S_SFR) && (aop->size > 1))
            {
              //if (offset>1) werrorfl (ic->filename, ic->lineno, E_SET_MISC,"Symbol >65535");
              if (offset>1) werror (E_INTERNAL_ERROR, __FILE__, __LINE__, "aopget sfr got offset>1, addr >65535");
              //dbuf_printf (&dbuf, "((%s >> %d) & 0xFF)", aop->aopu.aop_dir, offset * 8);
              if (offset==1) dbuf_printf (&dbuf, "hi_(%s)", aop->aopu.aop_dir);
              if (offset==0) dbuf_printf (&dbuf, "lo_(%s)", aop->aopu.aop_dir);
            }
          else if (offset)
            {
              D (emitcode (";", "aopGet3"));
        ...
        case AOP_LIT:
          if (opIsGptr (oper) && IS_FUNCPTR (operandType (oper)) && offset == GPTRSIZE - 1)
          ...
          else
            {
              ...
              D (emitcode (";", "aopGet5 %s",aopLiteralLong (aop->aopu.aop_lit, offset, size)));
            }
          break;
...
aopPut (operand * result, const char *s, int offset)
{
  ...
  switch (aop->type)
    ...
    case AOP_DIR:
      D (emitcode (";", "aopPut DIR"));
      if ((SPEC_SCLS (getSpec (operandType (result))) == S_SFR) && (aop->size > 1))
        {
          //if (offset>1) werrorfl (ic->filename, ic->lineno, E_SET_MISC,"Symbol >65535");
          if (offset>1) werror (E_INTERNAL_ERROR, __FILE__, __LINE__, "aopPut sfr offset > 1, addr>65535");
          //dbuf_printf (&dbuf, "((%s >> %d) & 0xFF)", aop->aopu.aop_dir, offset * 8);
          if (offset==0)  dbuf_printf (&dbuf, "lo_(%s)", aop->aopu.aop_dir);
          if (offset==1)  dbuf_printf (&dbuf, "hi_(%s)", aop->aopu.aop_dir);
        }
...
genPointerPush (iCode *ic)
{
  ...
  asmop *aop = 0;
  reg_info *preg = 0;
  if (p_type == POINTER || p_type == IPOINTER || p_type == SFRPOINTER)
    {
      aop = newAsmop (0);
...
genPointerPush (iCode *ic)
{
  ...
  if ((p_type == POINTER) || (p_type == SFRPOINTER))
    {
      pname=aopGet (left, 0, false, true);
    }
  else if (p_type == IPOINTER)
    {
    aop = newAsmop (0);
    wassert (preg = getFreePtr (ic, aop, false));
    emitcode ("mov", "%s,%s", preg->name, aopGet (left, 0, false, true));
  ...
  for (int i = 0; i < size; i++)
    {
      ...
      if (options.useXstack)
        ...
        {
          if (p_type != POINTER) emitpush ("acc");
        }
      if (i + 1 < size)
        if (p_type == POINTER)
          { }
        else if (p_type == SFRPOINTER)
          { }
        else if (p_type == IPOINTER)
          emitcode ("inc", "%s", preg->name);
        else
...
//                emitcode ("setb", "b[%d]", bit);
              emitcode ("setb", "bits_%d", bit);
              else
//                emitcode ("clr", "b[%d]", bit);
              emitcode ("clr", "bits_%d", bit);
            }
          else
            {
              /* we need to or */
...
//      emitcode ("mov", "acc, #(%s >> 8)", sym->rname);
      emitcode ("mov", "acc, #hi_(%s)", sym->rname);
      emitpush ("acc");
      emitcode ("mov", "acc, #0x%02x", pointerTypeToGPByte (pointerCode (getSpec (operandType (result))), 0, 0));
      emitpush ("acc");
    }
...
//  emitcode ("", ".section .text.code,\"ax\" ;code for function %s",sym->name);
  emitcode ("", ".section .text.code.%s,\"ax\" ;code for function %s",sym->name,sym->name);
  emitcode ("", ".type   %s, @function",sym->name);
...
//            }
  ...
  switch FUNC_REGBANK (ftype) {
    case -1 : emitcode ("", ".using 0");
    case 0 : emitcode ("", ".using 0"); break;
    case 1 : emitcode ("", ".using 1"); break;
    case 2 : emitcode ("", ".using 2"); break;
    case 3 : emitcode ("", ".using 3"); break;
    default: werror (E_NO_SUCH_BANK, FUNC_REGBANK (ftype));
  }
  ...
  if (runtimeSign)
    {
      ...
      emitcode ("setb", "%s",RegB_B0);
      else
        //emitcode ("clr", "F0"); /* reset sign flag */
      emitcode ("clr", "%s",RegB_B0);
    }
  ...
  if (runtimeSign || compiletimeSign)
    {
      ...
      emitcode ("jnb", "%s,!tlabel", RegB_B0,labelKey2num (lbl->key));
  ...
  if (runtimeSign)
    {
      if (compiletimeSign)
        //emitcode ("setb", "F0");        /* set sign flag */
      emitcode ("setb", "%s",RegB_B0);        /* set sign flag */
      else
        //emitcode ("clr", "F0"); /* reset sign flag */
      emitcode ("clr", "%s",RegB_B0);        /* set sign flag */
    }
  ...
  else                          /* ! literal */
    {
      if (rUnsigned)
      ...
      else
        {
          ...
          emitcode ("jnb", "acc.7,!tlabel", labelKey2num (lbl->key));
//          emitcode ("cpl", "F0");       /* complement sign flag */
          emitcode ("cpl", "%s",RegB_B0);       /* complement sign flag */
          emitcode ("cpl", "a");        /* 2's complement */
          emitcode ("inc", "a");
          emitLabel (lbl);
          emitcode ("mov", "b,a");
...
//          emitcode ("clr", "F0");       /* clear sign flag */
          emitcode ("clr", "%s",RegB_B0);       /* clear sign flag */
...
/*-----------------------------------------------------------------*/
static char *
genUnpackBits (operand * result, const char *rname, int p_type, iCode * ifx)
{
  ...
  crname = (char *) Safe_alloc (strlen(rname)+10);
  strcpy(crname,rname);
  ...
  for (rlen = blen; rlen >= 8; rlen -= 8)
    {
      ...
      if (rlen > 8)
        if (p_type!=POINTER)
          ...
          {
            sprintf(crname,"(%s+1)",rname);
          }
    }
  /* Handle the partial byte at the end */
  if (rlen)
    {
      emitPtrByteGet (crname, p_type, FALSE);
...
/*-----------------------------------------------------------------*/
/* genIndirectNearPointerGet - emitcode for near pointer fetch in Indirect  */
/*-----------------------------------------------------------------*/
static void
genNearIPointerGet (operand * left, operand * result, iCode * ic, iCode * pi, iCode * ifx)
{
  asmop *aop = NULL;
  reg_info *preg = NULL;
  const char *rname;
...
/*-----------------------------------------------------------------*/
static void
genNearSFRPointerGet (operand * left, operand * result, iCode * ic, iCode * pi, iCode * ifx)
{
  asmop *aop = NULL;
  reg_info *preg = NULL;
  ...
  D (emitcode (";", "genNearSFRPointerGet"));
  ...
  /* if left is rematerialisable and
     ...
     the left is pointer to data/sfr space */
  if (IS_AOP_IMMEDIATE (left) && !IS_BITFIELD (retype) && DCL_TYPE (ltype) == SFRPOINTER)
    {
      genDataPointerGet (left, result, ic);
      return;
    }
//  if (!IS_AOP_IMMEDIATE (left)) werror (E_SET_DIRECT, __FILE__, __LINE__, "genNearSFRPointerGet: !Immediate");
  if (!IS_AOP_IMMEDIATE (left)) D (emitcode (";", "genNearSFRPointerGet: !Immediate"));
  aopOp (result, ic, result ? TRUE : FALSE);
  rname = aopGet (left, 0, FALSE, FALSE);
  /* if bitfield then unpack the bits */
  if (IS_BITFIELD (retype))
    ifxCond = genUnpackBits (result, &rname[1], SFRPOINTER, ifx);
  else
    {
//      werror (E_SET_DIRECT, __FILE__, __LINE__, "genNearSFRPointerGet: !Bitfield");
      D (emitcode (";", "genNearSFRPointerGet: !Bitfield"));
    }
  if (ifx && !ifx->generated)
    {
      genIfxJump (ifx, ifxCond, left, NULL, result, ic->next);
...
//  if (AOP_TYPE (left) == AOP_IMMD && !IS_BITFIELD (retype) && ((DCL_TYPE (ltype) == POINTER) || (DCL_TYPE (ltype) == SFRPOINTER)))
   if (IS_AOP_IMMEDIATE (left) && !IS_BITFIELD (retype) && ((DCL_TYPE (ltype) == POINTER) || (DCL_TYPE (ltype) == SFRPOINTER)))
    {
      genDataPointerGet (left, result, ic);
      return;
    }
  ...
  if (!AOP_INPREG (AOP (left)))
    {
      if (IS_AOP_PREG (left))
        ...
        {
          D (emitcode (";", "genNearPointerGet requests free pointer"));
          /* otherwise get a free pointer register */
          aop = newAsmop (0);
          preg = getFreePtr (ic, aop, FALSE);
          emitcode ("mov", "%s,%s", preg->name, aopGet (left, 0, FALSE, TRUE));
...
/*-----------------------------------------------------------------*/
/* genIndirectPointerGet - emitcode for indirect pointer fetch             */
/*-----------------------------------------------------------------*/
static void
genIndirectPointerGet (operand * left, operand * result, iCode * ic, iCode * pi, iCode * ifx)
{
  asmop *aop = NULL;
  reg_info *preg = NULL;
  const char *rname;
...
//  sym_link *ltype = operandType (left);
  ...
  D (emitcode (";", "genIndirectPointerGet"));
...
genPointerGet (iCode * ic, iCode * pi, iCode * ifx)
{
  ...
  switch (p_type)
    {
    case SFRPOINTER:
      D (emitcode (";", "genPointerGet SFRPOINTER"));
      genNearSFRPointerGet (left, result, ic, pi, ifx);
      break;
    case POINTER:
      D (emitcode (";", "genPointerGet POINTER"));
      genNearPointerGet (left, result, ic, pi, ifx);
      break;
    case IPOINTER:
      D (emitcode (";", "genPointerGet IPOINTER"));
      genNearIPointerGet (left, result, ic, pi, ifx);
      break;
...
genPackBits (sym_link * etype, operand * right, const char *rname, int p_type)
{
      ...
      if (AOP_TYPE (right) == AOP_LIT)
        {
           ...
           */
          D (emitcode (";", "genPackBits AOP_LIT"));
          ...
          if ((blen == 1) && (p_type != GPOINTER))
            {
              ...
              if (AOP_TYPE (right) == AOP_CRY)
                ...
                {
                  D (emitcode (";", "genPackBits !AOP_CRY"));
                  MOVA (aopGet (right, 0, FALSE, FALSE));
                  emitcode ("rrc", "a");
                }
              emitPtrByteGet (rname, p_type, FALSE);
            ...
            {
              D (emitcode (";", "genPackBits 2"));
              bool pushedB;
              /* Case with a bitfield length < 8 and arbitrary source
               */
              MOVA (aopGet (right, 0, FALSE, FALSE));
              D (emitcode (";", "genPackBits 2A"));
              /* shift and mask source value */
              AccLsh (bstr);
              D (emitcode (";", "genPackBits 2B"));
              emitcode ("anl", "a,#!constbyte", (~mask) & 0xff);

              pushedB = pushB ();
              /* transfer A to B and get next byte */
              D (emitcode (";", "genPackBits 2C"));
              emitPtrByteGet (rname, p_type, TRUE);
              D (emitcode (";", "genPackBits 2D"));
              emitcode ("anl", "a,#!constbyte", mask);
              emitcode ("orl", "a,b");
              if (p_type == GPOINTER)
                emitpop ("b");
  ...
  D (emitcode (";", "genPackBits>=7 %d %d",rlen,blen));
  for (rlen = blen; rlen >= 8; rlen -= 8)
    {
  ...
  if (rlen)
    {
      ...
      if (AOP_TYPE (right) == AOP_LIT)
        {
          ...
          litval >>= (blen - rlen);
          litval &= (~mask) & 0xff;
          D (emitcode (";", "genPackBits litval %d ",litval));
          emitPtrByteGet (crname, p_type, FALSE);
          if ((mask | litval) != 0xff)
            emitcode ("anl", "a,#!constbyte", mask);
          if (litval)
      ...
      else
        {
          ...
          pushedB = pushB ();
          /* transfer A to B and get next byte */
          emitPtrByteGet (crname, p_type, TRUE);
      ...
      emitPtrByteSet (crname, p_type, "a");
    }
...
genNearPointerSet (operand * right, operand * result, iCode * ic, iCode * pi)
{
  ...
  aopOp (result, ic, FALSE);
  D (emitcode (";", "genNearPointerSet retype Bitfield %d",IS_BITFIELD (retype)));
  D (emitcode (";", "genNearPointerSet letype Bitfield %d",IS_BITFIELD (letype)));
  D (emitcode (";", "genNearPointerSet retype Bitvar %d",IS_BITVAR (retype)));
  D (emitcode (";", "genNearPointerSet letype Bitvar %d",IS_BITVAR (letype)));
  D (emitcode (";", "genNearPointerSet retype Literal %d",IS_OP_LITERAL (right)));
  D (emitcode (";", "genNearPointerSet letype Literal %d",IS_OP_LITERAL (result)));
  D (emitcode (";", "genNearPointerSet letype bit_field %d",bit_field));
  /* if the result is rematerializable &
     in data space & not a bit variable */
//  if (AOP_TYPE (result) == AOP_IMMD && DCL_TYPE (ptype) == POINTER && !IS_BITVAR (retype) && !IS_BITVAR (letype))
  //  if (AOP_TYPE (result) == AOP_IMMD && DCL_TYPE (ptype) == POINTER && !IS_BITVAR (retype) && !IS_BITVAR (letype))

  //  if (IS_AOP_IMMEDIATE (result) && DCL_TYPE (ptype) == POINTER && !IS_BITVAR (retype) && !IS_BITVAR (letype))
    if (AOP_TYPE (result) == AOP_IMMD && DCL_TYPE (ptype) == POINTER && !bit_field)
    {
      genDataPointerSet (right, result, ic);
      return;
    }
//  D (emitcode (";", "genNearPointerSet Not taken"));
//  fprintf(stderr,"genNearPointerSet Not taken\n");
//  exit(1);
  /* if the value is already in a pointer register
     then don't need anything more */
  if (!AOP_INPREG (AOP (result)))
    {
      if (IS_AOP_PREG (result))
        ...
        {
          D (emitcode (";", "genNearPointerSet requests free pointer"));
          /* otherwise get a free pointer register */
          aop = newAsmop (0);
          preg = getFreePtr (ic, aop, FALSE);
...
//  if ((IS_BITFIELD (retype) || IS_BITFIELD (letype)) && (IS_BITFIELD (letype) && !IS_OP_LITERAL (result)))
//  if (IS_OP_LITERAL (result) && IS_BITFIELD (retype))
//    {
...
/*-----------------------------------------------------------------*/
/* genNearSFRPointerSet - emitcode for near pointer put               */
/*-----------------------------------------------------------------*/
static void
genNearSFRPointerSet (operand * right, operand * result, iCode * ic, iCode * pi)
{
  const char *rname=NULL;
  sym_link *retype, *letype;
  sym_link *ptype = operandType (result);
  bool bit_field = IS_BITVAR (operandType (result)->next);
  D (emitcode (";", "genNearSFRPointerSet"));
  ...
  /* if the result is rematerializable &
     in data space & not a bit variable */
  /* for near it should work also on literals and strings */
  D (emitcode (";", "genNearSFRPointerSet retype Bitfield %d",IS_BITFIELD (retype)));
  D (emitcode (";", "genNearSFRPointerSet letype Bitfield %d",IS_BITFIELD (letype)));
  D (emitcode (";", "genNearSFRPointerSet retype Bitvar %d",IS_BITVAR (retype)));
  D (emitcode (";", "genNearSFRPointerSet letype Bitvar %d",IS_BITVAR (letype)));
  D (emitcode (";", "genNearSFRPointerSet retype Bitvar %d",IS_OP_LITERAL (right)));
  D (emitcode (";", "genNearSFRPointerSet letype Bitvar %d",IS_OP_LITERAL (result)));
  D (emitcode (";", "genNearSFRPointerSet letype bit_field %d",bit_field));

//  if (IS_AOP_IMMEDIATE (result) && DCL_TYPE (ptype) == SFRPOINTER && !IS_BITVAR (retype) && !IS_BITVAR (letype))
  if (IS_AOP_IMMEDIATE (result) && DCL_TYPE (ptype) == SFRPOINTER && !bit_field)
    {
      genDataPointerSet (right, result, ic);
      return;
    }

//  if (!IS_AOP_IMMEDIATE (result)) werror (E_SET_DIRECT, __FILE__, __LINE__, "genNearSFRPointerSet: !immediate");
  if (!IS_AOP_IMMEDIATE (result)) D (emitcode (";", "genNearSFRPointerSet: !immediate"));
...
//  if ((IS_BITFIELD (retype) || IS_BITFIELD (letype)) && (IS_BITFIELD (letype) && !IS_OP_LITERAL (result)))
    {
      genPackBits ((IS_BITFIELD (retype) ? retype : letype), right, &rname[1], SFRPOINTER);
    }
//  if (bit_field)
//  {
//    //genPackBits ((IS_BITFIELD (retype) ? retype : letype), right, rname, POINTER);
////      genPackBits ((IS_BITFIELD (retype) ? retype : letype), right, &rname[1], SFRPOINTER);
//    genPackBits (operandType (result)->next, right, rname[1], SFRPOINTER);
//  }
//  else if (IS_BITFIELD (retype) || IS_BITFIELD (letype))
//    {
//     genPackBits ((IS_BITFIELD (retype) ? retype : letype), right, &rname[1], SFRPOINTER);
//    }
//  else
//    {
//      D (emitcode (";", "genNearSFRPointerSet: !BITFIELD"));
////      werror (E_SET_DIRECT, __FILE__, __LINE__, "genNearSFRPointerSet: !BITFIELD");
//      genPackBits (operandType (result)->next, right, rname[1], SFRPOINTER);
//    }
  Safe_free ((void *) rname);
...
/*-----------------------------------------------------------------*/
static void
genNearIPointerSet (operand * right, operand * result, iCode * ic, iCode * pi)
{
  asmop *aop = NULL;
  reg_info *preg = NULL;
  const char *rname;
...
//  if (IS_BITFIELD (retype) || IS_BITFIELD (letype))
//  if ((IS_BITFIELD (retype) || IS_BITFIELD (letype)) && (IS_BITFIELD (letype) && !IS_OP_LITERAL (result)))
//    genPackBits ((IS_BITFIELD (retype) ? retype : letype), right, rname, IPOINTER);
  if (bit_field)
...
genPointerSet (iCode * ic, iCode * pi)
{
  ...
  switch (p_type)
    {
    case SFRPOINTER:
      D (emitcode (";", "genPointerSet SFRPOINTER"));
      genNearSFRPointerSet (right, result, ic, pi);
      break;
    case POINTER:
      D (emitcode (";", "genPointerSet POINTER"));
      genNearPointerSet (right, result, ic, pi);
      break;
    case IPOINTER:
      D (emitcode (";", "genPointerSet IPOINTER"));
      genNearIPointerSet (right, result, ic, pi);
      break;
      ...
      if (offset)
        {
          ...
          //if (offset>1) werrorfl (ic->filename, ic->lineno, E_MISC,"address>65535 ");
          if (offset>1) werror (E_INTERNAL_ERROR, __FILE__, __LINE__, "genAddrOf is >1, address>65535");
        }
      else
        {
...
genAssign (iCode * ic)
{
  ...
  if ((size > 1) && (AOP_TYPE (result) != AOP_REG) &&   /* for registers too? (regression test passes) */
  ...
  else
    {
      ...
      while (size--)
        {
          D (emitcode (";", "genAssign4B"));
          aopPut (result, aopGet (right, offset, FALSE, FALSE), offset);
          offset++;
        }
    }
  D (emitcode (";", "genAssign5"));
  adjustArithmeticResult (ic);
  D (emitcode (";", "genAssign6"));
release:
  freeAsmop (result, NULL, ic, TRUE);
  freeAsmop (right, NULL, ic, TRUE);
}
